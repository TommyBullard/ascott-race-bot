...
test('createRacingApiClient: real client is constructable without credentials', () => {
  ...
  // The basic same-day results method exists for the preferred fallback path.
  assert.equal(typeof createRacingApiClient().getTodayResults, 'function');
...
test('getTodayResults: GETs /v1/results/today with region/limit/skip + Basic auth', async () => {
  const prevUser = process.env.RACING_API_USER;
  const prevKey = process.env.RACING_API_KEY;
  process.env.RACING_API_USER = 'u';
  process.env.RACING_API_KEY = 'k';
  ...
  try {
    const client = createRacingApiClient(fakeFetch, 0);
    await client.getTodayResults({ regionCodes: ['gb', 'ire'], limit: 100, skip: 50 });
    assert.match(capturedUrl, /\/v1\/results\/today\?/);
    assert.doesNotMatch(capturedUrl, /\/v1\/results\/today\/free/);
    assert.match(capturedUrl, /region=gb/);
    assert.match(capturedUrl, /region=ire/);
    assert.match(capturedUrl, /limit=100/);
    assert.match(capturedUrl, /skip=50/);
