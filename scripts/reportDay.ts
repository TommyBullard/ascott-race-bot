/**
 * CLI (READ-ONLY): generate a Markdown end-of-day race report from stored data.
 *
 * For each race on a meeting day (optionally one course) it joins the model's
 * FINAL PRE-OFF run — the latest `model_runs` row with `run_time <= off_time` —
 * to the official result (`runners.finish_pos`), the stored rank-1
 * recommendation (odds / stake / EV / confidence), the market favourite, the
 * alternatives, and the persisted data-quality / tipster observability. It then
 * renders a deterministic Markdown report with a day summary, a factual pattern
 * analysis, and an interpretation section. The pre-off run is chosen with the
 * same pure `selectPreOffRun` the dashboard/accuracy use, so post-off reruns are
 * ignored.
 *
 * Usage:
 ...
 *   reports/day-report-2026-06-16-ascot.md
 *
...
/** Builds one race's end-of-day record from stored data. Read-only; never writes. */
async function buildRaceRecord(race: RaceRow): Promise<DayReportRace> {
  const raceId = String(race.id);
  const offTime = race.off_time;

  const base: DayReportRace = {
    race_id: raceId,
  ...
  // Runner names + official finishing positions for the race (read-only). These
  // are needed even when there is no pre-off run, so the winner can be shown.
  ...
  // The official winner = the runner with finish_pos = 1 (deterministic by id on
  // the unexpected case of more than one). has_result mirrors the importer rule:
  // a race is "resulted" once a finish_pos = 1 runner is present.
  const winner = runners
    .filter((r) => toNumberOrNull(r.finish_pos) === 1)
    .sort((a, b) => String(a.id).localeCompare(String(b.id)))[0];
  if (winner) {
    base.has_result = true;
    base.winner_name = winner.horse_name;
  }
  ...
  // Per-runner scores, the rank-1 rec, and the run's stored quote odds.
  const [scoresRes, recRes, quotesRes] = await Promise.all([
    supabaseAdmin
  ...
  for (const res of [scoresRes, recRes, quotesRes]) {
    if (res.error) {
      throw new Error(`Failed to read report data for race ${raceId}: ${res.error.message}`);
    }
  }
  ...
  const toRunner = (s: ScoreRow): DayReportRunner => {
...
async function main(): Promise<void> {
  ...
  const args = parseDayReportArgs(process.argv.slice(2));
  if (!args.date) {
    console.error(
      'Usage: npm run report:day -- --date YYYY-MM-DD [--course <name>]\n' +
  ...
  // Build each race record; isolate per-race failures so one bad race can't sink
  // the whole report.
  const records: DayReportRace[] = [];
  for (const race of races) {
    try {
      records.push(await buildRaceRecord(race));
    } catch (err) {
      console.error(
  ...
  const markdown = renderDayReportMarkdown(report);
  const outPath = buildDayReportPath(args.date, args.course);
  mkdirSync(dirname(outPath), { recursive: true });
  writeFileSync(outPath, markdown, 'utf8');

  console.log(`End-of-day report written (read-only DB): ${outPath}`);
  console.log(`  races: ${records.length}${wantCourse ? ` (course ~ "${args.course}")` : ''}`);
}
