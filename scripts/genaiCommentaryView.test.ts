...
test('view: hasAny + disclaimer wiring', () => {
...
test('panel: read-only — no write controls, no fetch, renders disclaimer + hides when empty', () => {
  // Require real JSX usage (onClick=, <button) so prose like "no onClick" in the
  ...
  assert.match(PANEL_SRC, /return null/); // hides when there is no approved commentary
